...
class ReplayBuffer:
    ...
    def sample(self):
        ...
        ]  # random lookup in a deque is more efficient
    ...
    def save(self, fname):
        ...
        pickle.dump(self.memory, open("{}.pkl".format(fname), "wb"))

    def load(self, fname):
        """Load a replay buffer from disk."""
        self.memory = pickle.load(open("{}.pkl".format(fname), "rb"))
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def sample(self, beta=0.4):
        ...
        indices = np.random.choice(
            len(self.memory), size=self.batch_size, replace=True, p=probs
        )
        experiences = [
            self.memory[i] for i in indices
        ]  # random lookup in a deque is more efficient
    ...
    def save(self, fname):
        super(ReplayBuffer, self).save(fname)
        pickle.dump(self.priorities, open("{}_priorities.pkl".format(fname), "wb"))

    def load(self, fname):
        super(ReplayBuffer, self).load(fname)
...
class RecurrentPrioritizedReplayBuffer(PrioritizedReplayBuffer):
    ...
    def __init__(self, buffer_size, batch_size, prob_alpha=0.6, history_length=10):
        super(RecurrentPrioritizedReplayBuffer, self).__init__(
            buffer_size, batch_size, prob_alpha
        )
        self.history_length = (
            history_length - 1
        )  # we want the full input to be of length history length, so we subtract 1
    ...
    def sample(self, beta=0.4):
        # sample normally
        batch, weights, indices = super(RecurrentPrioritizedReplayBuffer, self).sample(
            beta
        )
